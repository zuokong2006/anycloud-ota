        ...
        const uint8_t *inbuf, uint8_t inlen)
{
    ...
    if (inlen > (sizeof(buf) - 1) || align > (sizeof(buf) - 1)) {
        return -1;
    }
    ...
    memset(buf, erased_val, sizeof(buf));
                     ...
                     uint8_t image_num)
{
    ...
    BOOT_LOG_DBG("CY_BOOT_BOOTLOADER_SIZE:0x%x\r\n", CY_BOOT_BOOTLOADER_SIZE);
    BOOT_LOG_DBG("CY_BOOT_PRIMARY_1_SIZE:0x%x\r\n", CY_BOOT_PRIMARY_1_SIZE);
